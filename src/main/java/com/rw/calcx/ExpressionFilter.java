...
public class ExpressionFilter implements UnaryOperator<TextFormatter.Change> {
    ...
    public Change apply(TextFormatter.Change change) { // return change to accept changes and null to reject
        ...
        if (changeData.matches(ACCEPTABLE_DATA)) {
            ...
            if (!changeData.isEmpty()) {
                ...
                } else if (input == CalcX.DECIMAL_SEPARATOR.charAt(0)) {
                ...
                } else if (input == CalcX.DEL.charAt(0)) {
                    if (!oldData.isEmpty()) {
                        return null;
                    }
                } else if (input == 'E') {
        ...
        if (text.isEmpty()) {
            ...
            for (int i = text.length() - 1; i >= 0; i--) {
                if (!Character.isDigit(text.charAt(i))) {
                    if (text.charAt(i) == CalcX.DECIMAL_SEPARATOR.charAt(0)) {
                        return false;
                    }
                    break;
                }
            }
            if (CalcXUtils.checkLastCharIsDigit(text) || CalcXUtils.checkLastCharIsOperator(text)) {
                return true;
            }
            return false;
        }
    ...
    private boolean validOperator(String text, char operator) {
        if (text.isEmpty() || (CalcXUtils.getLastChar(text) == '(')) {
            if (CalcXUtils.isSignOperator(operator)) {
                return true;
            }
            ...
            if (CalcXUtils.chechBeforeLastIsOperator(text)) {
